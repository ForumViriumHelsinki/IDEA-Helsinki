...
class IntersectionDetector:
    ...
    def buffer_segments(self, gdf: geopandas.GeoDataFrame, buffer_distance: float, buffering_crs: str | None = None) -> geopandas.GeoDataFrame:
        """
        ...
        if buffering_crs and original_crs != buffering_crs:
            self.logger.info(f"Projecting data from {original_crs} to {buffering_crs} for buffering...")
        ...
        # Apply buffering
        buffered_gdf['geometry'] = buffered_gdf.geometry.buffer(buffer_distance, cap_style="flat")

        # Project back to original CRS if needed.
        if buffering_crs and original_crs != buffering_crs:
            self.logger.info(f"Projecting data back to {original_crs} after buffering...")
            buffered_gdf = buffered_gdf.to_crs(original_crs)

        self.logger.info(f"Buffered {len(buffered_gdf)} segments by {buffer_distance}. Original geometries backed up.")

        return buffered_gdf

    def restore_original_geometries(self, gdf: geopandas.GeoDataFrame) -> geopandas.GeoDataFrame:
        """
        ...
        if 'geometry_original' in restored_gdf.columns:
            ...
            self.logger.warning("Column 'geometry_original' not found. Cannot restore geometries.")
