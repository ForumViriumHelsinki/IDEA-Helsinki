...
def validate_tomtom_aggregation_file(data: dict) -> dict:
    ...
    for key, value in segment_ids.items():
        if not key.isdigit():
            logger.warning("Dictionary key is not an Integer (segment ID), returning an empty dictionary!")
            return {}
        if not isinstance(value, dict):
            logger.warning("Dictionary key has no dictionary associated to it, returning an empty dictionary!")
            return {}

        if "geometry" not in value or "detailedSegment" not in value:
            logger.warning("Dictionaries geometry and detailedSegment keys missing, returning an empty dictionary!")
            return {}

        geometry = value.get("geometry")
        if not isinstance(geometry, dict):
            logger.warning("Dictionaries geometry key has no dictionary associated to it, returning an empty dictionary!")
            return {}
        if geometry.get("type") != "LineString":
            logger.warning("Dictionaries geometry is not 'LineString' format, returning an empty dictionary!")
            return {}
    ...
    for segment_data in detailed_segments:
        ...
        if isinstance(shape_data, list):
            for point in shape_data:
                if isinstance(point, dict) and "longitude" in point and "latitude" in point:
                    try:
                        shape_coords.append([float(point["longitude"]), float(point["latitude"])])
                    except (ValueError, TypeError):
                        logger.warning(f"Invalid coordinate data for segment {segment_id_str} in file '{file_name_for_log}'. Point: {point}")
                else:
                    logger.warning(f"Malformed shape point for segment {segment_id_str} in file '{file_name_for_log}'. Point: {point}")
        else:
        ...
        time_variant_payload = {
            "date": {
                tomtom_timestamp_str: {
                    "properties" : current_segment_properties
                }
            }
        }

        current_segment_geometry = {
            "type": "LineString",
...
def sort_tomtom_data_aggregation_file_by_date(tomtom_aggregation_file: dict) -> dict:
    ...
    """
    for segment_id, segment_data in tomtom_aggregation_file.get("segmentId", {}).items():
        if "detailedSegment" in segment_data and "date" in segment_data["detailedSegment"]:
            try:
                sorted_dates = sorted(segment_data["detailedSegment"]["date"].items(),key=lambda item: datetime.strptime(item[0], "%Y-%m-%dT%H:%M:%S"),)
                segment_data["detailedSegment"]["date"] = dict(sorted_dates)
            except ValueError as e:
                logger.warning(f"Could not sort dates for segment {segment_id} due to parsing error: {e}")

    return tomtom_aggregation_file


def update_tomtom_json_data_for_aggregation_file(new_tomtom_file: dict,tomtom_file_to_update: dict) -> dict:
    """
    ...
    if not tomtom_file_to_update:
        ...
        for segment_id_str, segment_values in new_tomtom_file["segmentId"].items():
            ...
            if segment_id_str not in tomtom_file_to_update["segmentId"]:
                ...
                tomtom_file_to_update["segmentId"][segment_id_str]["geometry"] = segment_values["geometry"]
                tomtom_file_to_update["segmentId"][segment_id_str]["detailedSegment"] = {}
                tomtom_file_to_update["segmentId"][segment_id_str]["detailedSegment"]["date"] = {}

            for date_key, date_value in new_tomtom_file["segmentId"][segment_id_str]["detailedSegment"]["date"].items():
                if date_key not in tomtom_file_to_update["segmentId"][segment_id_str]["detailedSegment"]["date"]:
                    tomtom_file_to_update["segmentId"][segment_id_str]["detailedSegment"]["date"][date_key] = date_value
                    num_updated_tomtom_records += 1

    if num_tomtom_records == 0:
        logger.info("updatable file empty, returning new Tom Tom file as base for future aggregation")
    else:
        logger.info(f"Tom Tom aggregation file updated. {num_tomtom_records} records scanned, of which {num_updated_tomtom_records} contained new records.")
