...
# URL substrings that identify internal InfluxDB endpoints.
# Matches both k8s service names (containing "influxdb") and
# local dev (localhost:8086, the InfluxDB default port).
_INFLUXDB_URL_PATTERNS: tuple[str, ...] = ("influxdb", ":8086")
...
def _filter_breadcrumb(breadcrumb: dict, hint: dict) -> dict | None:  # noqa: ARG001
    """Filter out noisy breadcrumbs from routine internal HTTP requests.

    Drops httplib breadcrumbs for successful (2xx) requests to InfluxDB
    endpoints. These are high-volume, low-value breadcrumbs generated by
    periodic health checks, queries, and data writes.

    Args:
        breadcrumb: The breadcrumb dict from Sentry SDK.
        hint: Additional context (unused, required by Sentry callback API).

    Returns:
        The breadcrumb to keep, or None to discard it.
    """
    if breadcrumb.get("category") != "httplib":
        return breadcrumb

    if breadcrumb.get("level") != "info":
        return breadcrumb
    ...
    status_code = data.get("http.response.status_code")
