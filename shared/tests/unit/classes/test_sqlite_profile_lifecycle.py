...
class TestSQLiteProfileLifecycle:
    @pytest.mark.asyncio
    @patch("idea_shared.classes.IdeaHelsinkiRoadSegment.IdeaHelsinkiRoadSegment._IdeaHelsinkiRoadSegment__get_hourly_profile_data", new_callable=AsyncMock)
    ...
    async def test_generation_saves_to_sqlite_and_drops_memory(
        ...
        with patch.object(segment, "_IdeaHelsinkiRoadSegment__get_idea_formated_segment_data_from_influxdb", new_callable=AsyncMock) as mock_get_val:
    ...
    def test_update_segment_deletes_profile_from_sqlite(self, mock_profile_repo):
        ...
        new_disturbances = [{"properties": {"tyo_alkaa": "2025-05-05", "tyo_paattyy": "2025-06-06"}}]
        
        # Call update
        segment.update_segment(new_disturbances)
